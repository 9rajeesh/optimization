...
public class Validate4  {
	...
	public static void main(String[] args) {
		...
		Double b1 =  state.evaluate(0,1,2,3,4,6,10,18,19,20,23);
		Double c1 =  state.evaluate(1,2,3,5,8,11,9,19,12,13,14,15,19);
		Double d1 =  state.evaluate(4,5,6,9,10,11,13,14,18,20,21);
		Double e1 =  state.evaluate(0,1,2,3,9,12,15,17,13,16,17,19,20);
		Double f1 =  state.evaluate(0,5,6,7,8,10,11,12,17,9,13,18,19);
		Double g1 =  state.evaluate(0,1,2,8,10,11,12,16,17,18,19);
		Double h1 =  state.evaluate(0,1,4,5,9,10,11,12,15,16,17,20);
		Double i1 =  state.evaluate(0,1,3,5,6,7,8,9,10,12,13,15,18);
		Double j1 =  state.evaluate(0,1,2,8,7,9,11,12,15,18,19,21);
		
		Double k1 =  state.evaluate(0,1,2,8,9,12,9,8,17,18,19);
		...
		Double v1 =  state.evaluate(0,1,4,5,6,8,12,13,19,20,22);
		...
		double spikesum =   (a1+b1+c1+d1)*(e1+f1+g1+h1)
		                  + (i1+j1+k1+l1)*(m1+n1+o1+p1) 
		                  + (q1+r1+s1+t1)*(u1+v1+w1+x1);
		
		if(s.charAt(count)=='0') {
		sum0 = sum0 + sum;
		sumsq0 = sumsq0 + Math.pow(sum, 1);
