...
public class Validate_symmetric_higher {
	...
	public static void main(String[] args) {
		...
		//int count = 1;
		//Long startIndex = 1024L*1024L+1024*512;
		//Long endIndex = 1024L*1024L*2 ;
		
		//Long startIndex = 1024L*1024L+1024*512+1024*64+1024*8+1024*4+1024+512+128+32+16;
		//Long endIndex = 1024L*1024L*2 - 1024*256 - 1024*128-1024*32-1024*16-1024*2-256-64-8;
		...
		Double a1 =  state.evaluate(0,2,4,  8,10,12, 15,18,19);
		Double b1 =  state.evaluate(1,3,4,  9,11,12, 16,18,19 );
		Double c1 =  state.evaluate(2,5,7,  8,10,11, 12,13,14);
		Double d1 =  state.evaluate(5,7,8,   9,12,13, 16,17,18);
		Double e1 =  state.evaluate(1,3,4,   5,9,10,  12,15,17);
		
		Double f1 =  state.evaluate(3,4,8,  9,11,12, 15,16,17);
		Double g1 =  state.evaluate(4,5,8,  9,12,13,  15,17,19);
		Double h1 =  state.evaluate(2,5,8,   11,14,15,  16,17,19);
		
		Double i1 =  state.evaluate(1,6,8,  9,11,13,  14,17,18);
		Double j1 =  state.evaluate(2,6,8,   19,12,15,  16,17,19);
		
		Double k1 =  state.evaluate(1,5,9,  10,12,13,  15,18,19);
		Double l1 =  state.evaluate(0,6,8,   11,12,13,  14,17,19);
		
		Double m1 =  state.evaluate(1,2,4,  5,8,9,  10,12,13);
		Double n1 =  state.evaluate(0,1,4,   5,6,8,  15,18,19);
		...
		*/
		Double sum = a1+b1+c1+d1+e1+f1+g1+h1+i1+j1+k1+l1+m1+n1;
		...
		System.out.println("prevsum0 is:"+prevSum02);
		System.out.println("prevsum1 is:"+prevSum12);
